 ...
 public:
  lidarImageProjection() {
    ...
    sync = new message_filters::Synchronizer<SyncPolicy>(
        SyncPolicy(10), *cloud_sub, *image_sub);
    sync->registerCallback(
        boost::bind(&lidarImageProjection::callback, this, _1, _2));
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr planeFilter(
    ...
    pcl::copyPointCloud<pcl::PointXYZ>(*cloud_filtered_y, inliers_indicies,
                                       *plane);
  ...
  void colorLidarPointsOnImage(double min_range, double max_range) {
    for (size_t i = 0; i < imagePoints.size(); i++) {
      ...
      cv::circle(image_in, imagePoints[i], 2, CV_RGB(red_field, green_field, 0),
                 -1, 1, 0);
    }
  }
  ...
  void callback(const sensor_msgs::PointCloud2ConstPtr &cloud_msg,
    ...
    if (project_only_plane) {
      ...
      for (size_t i = 0; i < in_cloud->points.size(); i++) {
        objectPoints_L.push_back(cv::Point3d(in_cloud->points[i].x,
                                             in_cloud->points[i].y,
                                             in_cloud->points[i].z));
      }
      cv::projectPoints(objectPoints_L, rvec, tvec, projection_matrix,
                        distCoeff, imagePoints, cv::noArray());
    } else {
